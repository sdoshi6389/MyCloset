"""Gymshark men's scraper — Shopify site (products.json is 404, so DOM scraping).
Collection URLs discovered from /pages/shop-men filtered to /mens paths.
...
HUB   = f"{BASE}/pages/shop-men"

TEST = "--test" in sys.argv  # python gymshark_men.py --test  (1 collection, 3 PDPs)
...
def get_collection_links(page):
    p(f"Finding men's collections from {HUB}...")
    ...
    links = page.evaluate("""() => {
        return Array.from(document.querySelectorAll('a[href*="/collections/"]'))
            ...
            .filter(h => h.includes('/mens') || h.includes('mens-'));
    }""")
    links = list(set(links))
    p(f"Found {len(links)} men's collections")
    return links
    ...
    try:
        ...
        return page.evaluate("""(pageUrl) => {
            ...
            const rows = swatches.map(s => {
                ...
                return {title, price, color, url: s.href.split('?')[0], image, images, gender: 'men'};
            }).filter(r => r.url && r.title);

            return rows.length ? rows : [{title, price, color: '', url: pageUrl, image, images, gender: 'men'}];
        }""", url)
...
def main():
    ensure_table(TABLE)
    p(f"Gymshark men's scraper -> {TABLE}" + (" [TEST MODE]" if TEST else ""))
