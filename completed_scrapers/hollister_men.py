"""Hollister men's scraper — discovers all category pages then scrapes each."""
import sys, os
...
def get_category_urls(page):
    """Discover all men's category URLs from the main men's landing page."""
    page.goto(f"{BASE}/shop/us/mens", timeout=60000)
    page.wait_for_timeout(5000)
    links = page.evaluate("""
        () => {
            const seen = new Set();
            document.querySelectorAll('a[href*="/shop/us/mens-"]').forEach(a => {
    ...
    """)
    # Exclude non-clothing and curated/marketing pages
    skip_keywords = ['cologne', 'body', 'fragrance', 'bestsellers', 'top-rated', 'new-arrivals']
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        p("Discovering men's categories...")
