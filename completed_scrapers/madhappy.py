"""Madhappy scraper — Shopify, color from option1, one row per unique color per product."""
import sys, os
...
TABLE = "products_madhappy"
BASE = "https://madhappy.com"
LIMIT = 250
...
def parse_products(raw):
    ...
    for prod in raw:
        ...
        if color_pos:
            seen_colors = {}
            for v in variants:
                color = (v.get(f"option{color_pos}") or '').strip().title()
