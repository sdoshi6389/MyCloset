"""Lounge Underwear scraper — Shopify products.json, title split on ' - '."""
import sys, os, json
...
TABLE = "products_lounge"
BASE = "https://lounge.com"
LIMIT = 250
...
def parse_products(raw_products):
    ...
    for prod in raw_products:
        ...
        # Title format: "Style Name - Color" e.g. "90s Straight-Leg Trousers - Navy"
        if ' - ' in full_title:
            idx = full_title.rfind(' - ')
            title = full_title[:idx].strip()
            color = full_title[idx + 3:].strip()
        else:
            title = full_title
            color = ''
