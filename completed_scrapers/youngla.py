"""YoungLA scraper — Shopify products.json, color from variant option1."""
import sys, os, json
...
TABLE = "products_youngla"
BASE = "https://www.youngla.com"
LIMIT = 250
    ...
    for prod in raw_products:
        ...
        for variant in variants:
            if color_idx is not None:
                color_raw = (variant.get(f"option{color_idx + 1}") or '').strip()
            else:
                color_raw = ''
            ...
            out.append({'title': title, 'price': price, 'color': color_raw, 'url': url, 'image': image})
