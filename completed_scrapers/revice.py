"""Revice Denim scraper — Shopify products.json via browser context."""
import sys, os, json
...
TABLE = "products_revice"
BASE = "https://www.revicedenim.com"
LIMIT = 250
...
def fetch_products_page(page, pg):
...
def parse_products(raw_products):
    ...
    for prod in raw_products:
        ...
        # Title format: "STYLE / COLOR" or just "STYLE"
        if ' / ' in full_title:
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        total_fetched = 0
        while True:
            raw = fetch_products_page(pg, page_num)
            if not raw:
            ...
            if len(raw) < LIMIT:
                p(f"  Last page reached ({len(raw)} < {LIMIT})")
                break
            page_num += 1
    ...
    p(f"\n{'='*50}")
    p(f"DONE! Total fetched: {total_fetched}")
    p(f"Total: {total_ins} inserted, {total_skip} skipped (duplicates)")
