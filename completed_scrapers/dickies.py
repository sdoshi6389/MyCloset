"""Dickies scraper — Shopify, color from option1 with (CODE) suffix stripped."""
import sys, os, re
...
def clean_color(raw):
    """Strip trailing (CODE) from color names like 'Dark Navy (0DN)'."""
...
def parse_products(raw):
    ...
    for prod in raw:
        ...
        if color_pos:
            # Deduplicate by color in case multiple size variants share a color
            seen_colors = {}
            for v in variants:
                color = clean_color(v.get(f"option{color_pos}") or '')
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        while True:
            ...
            total_ins += ins
            total_skip += skip
            p(f"Page {pg_num:2d}: {len(raw):3d} fetched -> {ins} inserted, {skip} skipped")
