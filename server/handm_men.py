...
START_URL = "https://www2.hm.com/en_us/men.html"
BASE_URL = "https://www2.hm.com"
BRAND = "hm_mens"

def get_category_links(page):
    print("🔍 Collecting all men category + seasonal links...")
    ...
    for a in sidebar_anchors:
        ...
        if href and "/en_us/men/products/" in href:
            category_links.add(BASE_URL + href)

    # === B. Seasonal/curated collection links ===
    seasonal_anchors = page.query_selector_all("a[href*='/en_us/men/seasonal-trending/']")
...
def scrape_product_detail(page, url):
    try:
        ...
        for swatch in color_swatches:
            ...
            color_href = swatch.get_attribute("href")
            full_href = BASE_URL + color_href if color_href and not color_href.startswith("http") else color_href
            if color_name:
                colors.append(f"{color_name}")
...
def main():
    ...
    # Save results
    output_csv = os.path.join(os.path.dirname(__file__), "hm_men_products.csv")
    df = pd.DataFrame(all_products)
    df.to_csv(output_csv, index=False)
    print(f"\n✅ Scraped {len(all_products)} products. Saved to {output_csv}")
